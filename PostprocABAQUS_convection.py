...
#            header += (H_field_name+'2,')
            ...
            for frame in frames:
                ...
                nset_val_U = U_field_comp.values[0]
                
                line = ''
                line += '%s,' % (nset_val_U.nodeLabel)
#                line += '%s,' % (x)
#                line += '%s,' % (y)
#                line += '%s,' % (z)
...
#                line += '%s,' % (E_field_comp.getScalarField(componentLabel=E_field_name+'11').values[0].data)
                line += '%s,' % (E_field_comp.getScalarField(componentLabel=E_field_name+'22').values[0].data)
...
#                line += '%s,'  % (E_field_comp.getScalarField(componentLabel=E_field_name+'13').values[0].data)
                line += '%s,'  % (E_field_comp.getScalarField(componentLabel=E_field_name+'23').values[0].data)
#                line += '%s,' % (S_field_comp.getScalarField(componentLabel=S_field_name+'11').values[0].data)
                line += '%s,' % (S_field_comp.getScalarField(componentLabel=S_field_name+'22').values[0].data)

...
from Data import SimulationData,ExperimentData,ExperimentLog,SimulationData3D
...
# calculate_data_fatigue_life
#==============================================================================
def calculate_data_fatigue_life_3D(data,material,fatigue_model):
    nodelabel = data.node_label[0]
    nth = data.axial_count_index_list[-2]
    nth = data.half_life_cycle
    time = data.obtainNthCycle('runing_time',nth)
    ...
    heat_flux_3 = data.obtainNthCycle('heat_flux_3',nth)
    length = len(time)
    s11 = data.obtainNthCycle('s11',nth)
    s22 = data.obtainNthCycle('s22',nth)
    s33 = data.obtainNthCycle('s33',nth)
    s12 = data.obtainNthCycle('s12',nth)
    s13 = data.obtainNthCycle('s13',nth)
    s23 = data.obtainNthCycle('s23',nth)
    e11 = data.obtainNthCycle('e11',nth)
    e22 = data.obtainNthCycle('e22',nth)
    e33 = data.obtainNthCycle('e33',nth)
    e12 = data.obtainNthCycle('e12',nth)/2.0
    e13 = data.obtainNthCycle('e13',nth)/2.0
    e23 = data.obtainNthCycle('e23',nth)/2.0
    if heat_flux_1 == []:
        heat_flux_1 = np.zeros(length)
    if heat_flux_2 == []:
        heat_flux_2 = np.zeros(length)
    ...
    for i in range(len(time)):
        ...
        strain.append([[e11[i],e12[i],e13[i]],[e12[i],e22[i],e23[i]],[e13[i],e23[i],e33[i]]])
        heatflux.append([heat_flux_1[i],heat_flux_2[i],heat_flux_3[i]])
    node = Node(nodelabel=nodelabel, dimension=3, time=time, coordinate=[], 
                displacement=[], stress=stress, strain=strain,
                temperature=temperature, heatflux=heatflux, theta_interval=1, phi_interval=1)
#    if fatigue_model == 'FS':
#        fatigue_data = node.fatigueLifeFSModel(material,k=1.0)
    if fatigue_model == 'FS':
        life = nth*2.0

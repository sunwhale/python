...
from Functions import obtain_kinematic_hardening_parameters,calculate_elastic_by_temperature_k4169
from Constants import *
from workbench import workbench

def calculate_umat_parameters_k4169(name='_output.txt'):
    parameters = []
    yield_stress = 300.0
    show = True
#    plastic_strain_list=[0.001,0.005,0.01,0.02]
    ...
    umat = UMAT(UMATDirectory = 'F:\\GitHub\\umat\\', 
                UMATMainFile = 'MAIN_K4169.for', 
                ParameterFortranFile = 'PARAMETERS_K4169_TMF.for',
...
#==============================================================================
    temperature = 300.0
    youngs_modulus, poisson_ratio, shear_modulus, y = calculate_elastic_by_temperature_k4169(temperature)
    ...
    cyclic_curve = [exp_cyclic.obtainNthCycle('axial_strain',10),exp_cyclic.obtainNthCycle('axial_stress',10)]
...
#==============================================================================
    ...
    monotonic_curve = [exp_monotonic.axial_log_strain,exp_monotonic.axial_true_stress]
    cyclic_curve = [exp_cyclic.obtainNthCycle('axial_strain',100),exp_cyclic.obtainNthCycle('axial_stress',100)]
    seg,zeta,r0,ri = obtain_kinematic_hardening_parameters(monotonic_curve,cyclic_curve,
                                                           position_x_list=plastic_strain_list,
                                                           yield_stress=yield_stress,
                                                           youngs_modulus=youngs_modulus,show=show)
...
#==============================================================================
    for i in range(seg):
        x=np.array([parameters[0][0],parameters[1][0]])
        y=np.array([parameters[0][3][i],parameters[1][3][i]])
        z=np.polyfit(x,y,1)
        p = np.poly1d(z)
...
#==============================================================================
    for i in range(seg):
        x=np.array([parameters[0][0],parameters[1][0]])
        y=np.array([parameters[0][4][i],parameters[1][4][i]])
        z=np.polyfit(x,y,1)
        p = np.poly1d(z)
...
calculate_umat_parameters_k4169()
#for name in ['7036']:
#    workbench(name)